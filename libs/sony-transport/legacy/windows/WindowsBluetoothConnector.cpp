...
int WindowsBluetoothConnector::send(char* buf, size_t length) {
    ...
    if (bytesSent == SOCKET_ERROR) {
        throw RecoverableException(
            "Couldn't send (" + std::to_string(WSAGetLastError()) + ")", true);
    }
    return bytesSent;
}
...
std::vector<BluetoothDevice> WindowsBluetoothConnector::getConnectedDevices() {
    ...
    if (!radioFindHandle) {
        if (ERROR_NO_MORE_ITEMS == GetLastError()) {
            ...
            throw std::runtime_error("BluetoothFindFirstRadio() failed with error code " +
    ...
    if (!BluetoothFindRadioClose(radioFindHandle))
        throw std::runtime_error(
            "BluetoothFindRadioClose(bt) failed with error code " + std::to_string(GetLastError()));
...
bool WindowsBluetoothConnector::isConnected() noexcept { return this->_connected; }
...
void WindowsBluetoothConnector::_initSocket() {
    ...
    if (::setsockopt(
            sock, SOL_RFCOMM, SO_BTH_ENCRYPT, reinterpret_cast<char*>(&enable), sizeof(enable))) {
        throw std::runtime_error(
    ...
    ::setsockopt(sock,
        SOL_SOCKET,
        SO_RCVTIMEO,
        reinterpret_cast<char*>(&recvTimeoutMs),
...
std::string WindowsBluetoothConnector::_wstringToUtf8(const std::wstring& wstr) {
    std::string strTo;
    const int num_chars = WideCharToMultiByte(
        CP_UTF8, 0, wstr.c_str(), static_cast<int>(wstr.length()), NULL, 0, NULL, NULL);
...
std::vector<BluetoothDevice> WindowsBluetoothConnector::_findDevicesInRadio(
    ...
    if (!dev_find_handle) {
        if (ERROR_NO_MORE_ITEMS == GetLastError()) {
            ...
            throw std::runtime_error("BluetoothFindFirstDevice() failed with error code: " +
                                     std::to_string(GetLastError()));
        }
    }
    ...
    do {
        res.emplace_back(BluetoothDevice{
            _wstringToUtf8(device_info.szName), MACBytesToString(device_info.Address.rgBytes)});
    ...
    if (!BluetoothFindDeviceClose(dev_find_handle))
        throw std::runtime_error("BluetoothFindDeviceClose(bt_dev) failed with error code: " +
                                 std::to_string(GetLastError()));
