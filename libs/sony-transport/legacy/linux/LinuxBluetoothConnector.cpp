...
void LinuxBluetoothConnector::connect(const std::string& addrStr) {
    ...
    if (channel == 0) {
        throw RecoverableException("Couldn't find the Sony service record on this device (neither "
                                   "protocol version) - is this a supported headset?",
            true);
    }
    ...
    for (auto& adapter : adapter_paths) {
        ...
        if (connected) {
            res.insert(res.begin(),
                {.name = name, .mac = address, .paired = paired, .connected = connected});
        } else {
...
bool LinuxBluetoothConnector::isConnected() noexcept { return this->_connected; }
