...
unsigned char _sumChecksum(const Buffer& src) { return _sumChecksum(src.data(), src.size()); }

Buffer packageDataForBt(const Buffer& src, DATA_TYPE dataType, unsigned int seqNumber) {
    SonyFrame frame{.type = static_cast<DataType>(dataType),
    ...
    try {
        ...
        if (e.code() == SonyErrorCode::InvalidFrame &&
            ...
            throw RecoverableException(
                "Invalid message: declared size exceeds received data", true);
        }
        throw;
    }
}
...
Buffer serializeNcAndAsmSetting(NC_ASM_EFFECT ncAsmEffect,
    ...
    ret.push_back(
        static_cast<unsigned char>(NC_ASM_INQUIRED_TYPE::NOISE_CANCELLING_AND_AMBIENT_SOUND_MODE));
...
Buffer serializeNcAndAsmSettingV2(NC_ASM_EFFECT ncAsmEffect,
    NC_ASM_SETTING_TYPE_V2 ncAsmSettingType,
    ASM_ID voicePassthrough,
    char asmLevel) {
