...
template <class T> class SingleInstanceFuture : public std::future<T> {
public:
...
inline void SingleInstanceFuture<T>::setFromAsync(Func func, Args&&... args) {
    if (this->valid()) {
        throw std::runtime_error(
            "The asynchronous action was cancelled before it finished executing");
    }
    *this = std::async(std::launch::async, func, std::forward<Args>(args)...);
}

template <class T> inline bool SingleInstanceFuture<T>::ready() {
    if (!this->valid()) {
        return false;
    }
...
template <class T>
inline SingleInstanceFuture<T>::SingleInstanceFuture(std::future<T> other)
