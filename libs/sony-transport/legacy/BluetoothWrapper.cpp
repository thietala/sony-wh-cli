...
bool BluetoothWrapper::isConnected() noexcept { return this->_connector->isConnected(); }
...
Buffer BluetoothWrapper::sendCommandAndReadResponse(
    const std::vector<char>& bytes, unsigned char retCommandId, int retSubType) {
    std::lock_guard guard(this->_connectorMtx);
    auto data = CommandSerializer::packageDataForBt(bytes, DATA_TYPE::DATA_MDR, this->_seqNumber++);
    ...
    for (int i = 0; i < 16; i++) {
        ...
        if (msg.dataType == DATA_TYPE::DATA_MDR && !msg.payload.empty() &&
            ...
            (retSubType < 0 ||
                (msg.payload.size() >= 2 && (unsigned char)msg.payload[1] == retSubType))) {
            return msg.payload;
        }
    }
...
CommandSerializer::Message BluetoothWrapper::_readMessage() {
    ...
    do {
        ...
        if (!this->_leftoverBytes.empty()) {
            ...
            this->_leftoverBytes.erase(
                this->_leftoverBytes.begin(), this->_leftoverBytes.begin() + numRecvd);
        } else {
            ...
            if (buf[i] == START_MARKER) {
                if (ongoingMessage) {
                    throw RecoverableException(
                        "Invalid: Multiple start markers without an end marker", true);
                }
                messageStart = i + 1;
                ongoingMessage = true;
    ...
    if (msg.dataType == DATA_TYPE::DATA_MDR) {
        auto ack = CommandSerializer::packageDataForBt(
            {}, DATA_TYPE::ACK, (unsigned char)(1 - msg.seqNumber));
