...
public:
    ...
    int
    getAutoPowerOff(); // index into the option list (0=Off,1=5m,2=30m,3=1h,4=3h,5=when taken off)
...
private:
    ...
    Property<SOUND_POSITION_PRESET> _surroundPosition = {
        SOUND_POSITION_PRESET::OUT_OF_RANGE, SOUND_POSITION_PRESET::OFF};
...
template <class T> inline void Property<T>::fulfill() { this->current = this->desired; }

template <class T> inline bool Property<T>::isFulfilled() { return this->desired == this->current; }
