...
void connectTo(int fd, const std::string& path, Clock::time_point deadline) {
    ...
    if (::connect(fd, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
        if (errno != EINPROGRESS && errno != EAGAIN)
            throw std::runtime_error("Daemon unavailable at " + path);
...
std::string IpcClient::request(std::string_view line, std::chrono::milliseconds timeout) {
    ...
    while (sent < data.size()) {
        ...
        if (n > 0)
            ...
            throw std::runtime_error("Cannot send to daemon");
    }
    std::string response;
    while (true) {
        waitFor(fd.value, POLLIN, deadline);
        char buf[4096];
        auto n = ::read(fd.value, buf, sizeof(buf));
        if (n == 0) throw std::runtime_error("Truncated daemon response");
        if (n < 0) {
            if (errno == EINTR || errno == EAGAIN) continue;
            throw std::runtime_error("Cannot read daemon response");
        }
    ...
    (void)line;
    (void)timeout;
    throw std::runtime_error("IPC not supported on this platform");
#endif
}
IpcResponse IpcClient::sendCommand(std::string_view line, std::chrono::milliseconds timeout) {
    try {
        return IpcProtocol::parseResponse(request(line, timeout));
