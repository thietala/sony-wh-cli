...
IpcServer::IpcServer(std::shared_ptr<IDeviceService> service,
    std::string path,
    std::chrono::milliseconds idleDisconnect)
...
void IpcServer::start() {
    ...
    try {
        ...
        _lockFd = ::open(
            (_socketPath + ".lock").c_str(), O_CREAT | O_RDWR | O_NOFOLLOW | O_CLOEXEC, 0600);
...
void IpcServer::stop() noexcept {
    ...
    struct stat st {};
    if (_socketInode && ::lstat(_socketPath.c_str(), &st) == 0 && st.st_ino == _socketInode &&
        st.st_uid == ::geteuid())
...
void IpcServer::_connectOnDemand(const std::string& line) {
    ...
    try {
        _service->connect(transport::DeviceAddress(devices.front().address), devices.front().name);
...
void IpcServer::_executeLoop() {
    while (_running) {
        std::shared_ptr<Job> job;
        {
            ...
            _queueCv.wait_for(lock, std::chrono::milliseconds(100), [this] {
                return !_running || !_jobs.empty();
            });
            if (!_running) break;
            if (!_jobs.empty()) {
                job = _jobs.front();
        ...
        if (job && !job->cancelled) {
            try {
                ...
                auto response =
                    _service ? IpcProtocol::executeLine(job->line, *_service)
                ...
                job->result.set_value("ERR|Internal service error|\n");
            }
        ...
        if (_running && idle && _service) {
            try {
                ...
                if (_service->isConnected() &&
                    std::chrono::steady_clock::now() - _lastActivity > _idleDisconnect)
                    _service->disconnect();
                else
                    _service->tick();
            } catch (...) {}
...
void IpcServer::_serverLoop() {
    ...
    while (_running) {
        ...
        for (auto& c : clients)
            fds.push_back({c.fd, static_cast<short>(c.output.empty() ? POLLIN : POLLOUT), 0});
        ...
        for (size_t i = clients.size(); i-- > 0;) {
            auto& c = clients[i];
            auto events = fds[i + 1].revents;
            bool close = Clock::now() >= c.deadline || (events & (POLLERR | POLLHUP | POLLNVAL));
            if (!close && c.result.valid() &&
                c.result.wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
                c.output = c.result.get();
            ...
            if (!close && (events & POLLIN)) {
                ...
                if (n > 0) {
                    ...
                    if (c.job)
                        ...
                        if (c.input.size() > 16 * 1024)
                            close = true;
                        else if (end != std::string::npos) {
                            if (end + 1 != c.input.size())
                                close = true;
                            else {
                                c.job = std::make_shared<Job>();
                                c.job->line = c.input.substr(0, end);
                                c.result = c.job->result.get_future();
                                std::lock_guard lock(_queueMutex);
                                if (_jobs.size() >= 16)
                                    c.job->result.set_value("ERR|Service busy|\n");
                                else {
                                    _jobs.push_back(c.job);
                                    _queueCv.notify_one();
                                }
        ...
        if (fds[0].revents & POLLIN) {
            ...
            if (fd >= 0) {
                ...
                if (clients.size() >= 16 || !unixsocket::sameUser(fd))
                    ::close(fd);
                else
