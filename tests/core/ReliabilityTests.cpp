...
TEST_CASE(
    "Lifecycle prefers connected candidates and honors explicit selection", "[core][recovery]") {
    auto transport = std::make_shared<ReplyTransport>();
    auto discovery = std::make_shared<FakeDeviceDiscovery>();
    discovery->setDevices({
        {"WH-1000XM3", DeviceAddress("11:22:33:44:55:66"), true, false},
    ...
    SECTION("Explicit address is the only candidate") {
        service.startAutoConnect("11:22:33:44:55:66");
        service.tick();
        CHECK(service.selectedAddress() == "11:22:33:44:55:66");
        CHECK(transport->attempts.size() == 1);
    }
...
TEST_CASE("Retry delay doubles and is capped at thirty seconds", "[core][recovery]") {
    ...
    service.startAutoConnect(transport->failAddress);
    service.tick();
    size_t count = 1;
    for (int delay : {1, 2, 4, 8, 16, 30, 30}) {
        now += std::chrono::milliseconds(delay * 1000 - 1);
        service.tick();
...
TEST_CASE("Structured snapshots are escaped versioned and truthful", "[core][json]") {
    ...
    CHECK(
        Json::parse(JsonProtocol::executeLine(deep, service))["error"]["code"] == "InvalidRequest");
    ...
    CHECK(JsonProtocol::execute(
              ...
              service)["error"]["code"] == "InvalidRequest");
    service.disconnect();
    ...
    CHECK(status.message == "Device disconnected");
}
TEST_CASE(
    "JSON factoryReset wipes the pairing so it requires explicit confirmation", "[core][json]") {
    auto transport = std::make_shared<ReplyTransport>();
    auto discovery = std::make_shared<FakeDeviceDiscovery>();
    DeviceService service(transport, discovery);
    service.connect(DeviceAddress("11:22:33:44:55:66"), "WH-1000XM5");
    ...
    auto factoryResetsSent = [&] {
        size_t count = 0;
        for (const auto& raw : transport->sentFrames())
            if (protocol::FrameCodec::decode(raw).payload == std::vector<uint8_t>{0xf8, 0x09, 0x01})
                ++count;
        return count;
    };
    auto call = [&](const Json& params) {
        return JsonProtocol::execute(
            {
                {"version", 1             },
                {"id",      1             },
                {"method",  "factoryReset"},
...
TEST_CASE("Notification callbacks may read state from another thread", "[core][events]") {
    auto transport = std::make_shared<ReplyTransport>();
    SonyDevice device(transport);
    device.connect(DeviceAddress("11:22:33:44:55:66"), "WH-1000XM5");
    std::promise<bool> completed;
    auto result = completed.get_future();
    auto id = device.events().onStateChanged([&](const auto&) {
        auto read = std::async(
            std::launch::async, [&] { return device.snapshot()->noiseControl.ambientLevel; });
    ...
    CHECK(result.get());
    device.events().removeListener(id);
}
#ifndef _WIN32
namespace {
struct ClientFd {
...
TEST_CASE("IPC isolates idle oversized and disappearing clients", "[core][ipc][security]") {
...
#endif
        pollfd pfd{attacker.fd, POLLIN, 0};
        REQUIRE(::poll(&pfd, 1, 2000) > 0);
        char b;
        CHECK(::read(attacker.fd, &b, 1) <= 0);
        CHECK(client.sendCommand("devices").success);
    }
    SECTION("Client disappears before response") {
        {
            ClientFd vanished(socket.path);
            ssize_t written = ::write(vanished.fd, "devices\n", 8);
            (void)written;
        }
        CHECK(client.sendCommand("devices").success);
    }
    ...
    SECTION("Second daemon cannot evict the first") {
        IpcServer second(service, socket.path);
        CHECK_THROWS(second.start());
        second.stop();
        CHECK(client.sendCommand("devices").success);
    }
    struct stat st {};
    REQUIRE(::lstat(socket.path.c_str(), &st) == 0);
    CHECK((st.st_mode & 0777) == 0600);
}
TEST_CASE(
    "IPC refuses insecure paths and recovers an owned stale socket", "[core][ipc][security]") {
    PrivateSocket socket;
    auto service = std::make_shared<DeviceService>(std::make_shared<FakeTransport>());
