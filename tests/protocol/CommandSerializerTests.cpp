...
Buffer body(const Buffer& frame) { return Buffer(frame.begin() + 1, frame.end() - 1); }
} // namespace
...
TEST_CASE("MDR encoding matches a literal wire frame", "[serializer][framing]") {
    ...
    const auto encoded =
        CommandSerializer::packageDataForBt(bytes({0x66, 0x02}), DATA_TYPE::DATA_MDR, 1);
...
TEST_CASE("Reserved payload bytes have literal escape pairs", "[serializer][escaping]") {
    ...
    REQUIRE(
        CommandSerializer::_escapeSpecials(payload) == bytes({0x3d, 0x2c, 0x3d, 0x2d, 0x3d, 0x2e}));
    const auto encoded = CommandSerializer::packageDataForBt(payload, DATA_TYPE::DATA_MDR, 0);
    REQUIRE(encoded ==
            bytes({0x3e, 0x0c, 0, 0, 0, 0, 3, 0x3d, 0x2c, 0x3d, 0x2d, 0x3d, 0x2e, 0xc6, 0x3c}));
    REQUIRE(CommandSerializer::unpackBtMessage(body(encoded)).payload == payload);
}
...
TEST_CASE("All byte values survive escape and frame round trips", "[serializer][escaping]") {
    ...
    REQUIRE(CommandSerializer::_unescapeSpecials(CommandSerializer::_escapeSpecials(payload)) ==
            payload);
...
TEST_CASE("Escaping includes sequence length and checksum bytes", "[serializer][escaping]") {
    ...
    SECTION("Checksum") {
        ...
        const auto encoded = CommandSerializer::packageDataForBt(
            bytes({static_cast<unsigned int>(value)}), DATA_TYPE::DATA_MDR, 0);
        REQUIRE(encoded[encoded.size() - 3] == 0x3d);
        REQUIRE(encoded[encoded.size() - 2] == value - 3);
        REQUIRE(CommandSerializer::unpackBtMessage(body(encoded)).payload ==
                bytes({static_cast<unsigned int>(value)}));
    }
}
...
TEST_CASE("Malformed escape sequences are rejected", "[serializer][escaping]") {
    ...
    REQUIRE_THROWS_AS(
        CommandSerializer::_unescapeSpecials(bytes({0x11, 0x3d})), std::runtime_error);
    REQUIRE_THROWS_AS(
        CommandSerializer::_unescapeSpecials(bytes({0x3d, 0x00})), std::runtime_error);
    REQUIRE_THROWS_AS(
        CommandSerializer::_unescapeSpecials(bytes({0x3d, 0x3e})), std::runtime_error);
}
...
TEST_CASE("Literal valid body decodes without delimiters", "[serializer][parsing]") {
    const auto decoded =
        CommandSerializer::unpackBtMessage(bytes({0x0c, 1, 0, 0, 0, 2, 0x67, 2, 0x78}));
    REQUIRE(decoded.dataType == DATA_TYPE::DATA_MDR);
    REQUIRE(decoded.seqNumber == 1);
...
TEST_CASE("Every truncated prefix of a valid escaped body is rejected", "[serializer][parsing]") {
    ...
    for (size_t length = 0; length < complete.size(); ++length) {
        DYNAMIC_SECTION("Body prefix length " << length) {
            REQUIRE_THROWS_AS(CommandSerializer::unpackBtMessage(
                                  Buffer(complete.begin(), complete.begin() + length)),
                std::runtime_error);
        }
    }
}
...
TEST_CASE(
    "A shortened declaration with no matching checksum is rejected", "[serializer][parsing]") {
    REQUIRE_THROWS_AS(
        CommandSerializer::unpackBtMessage(bytes({0x0c, 0, 0, 0, 0, 1, 0x67, 2, 0x77})),
        RecoverableException);
}

TEST_CASE("Baseline body decoder ignores bytes after a valid declared checksum",
    "[serializer][baseline]") {
    // This permissive behavior is not a strict FrameCodec contract; see architecture-current.md.
    const auto decoded =
        CommandSerializer::unpackBtMessage(bytes({0x0c, 0, 0, 0, 0, 1, 0x67, 0x74, 0x55, 0x66}));
    REQUIRE(decoded.payload == bytes({0x67}));
}

TEST_CASE("Wire frames are not the body decoder input contract", "[serializer][parsing]") {
    REQUIRE_THROWS_AS(CommandSerializer::unpackBtMessage(bytes({0x3e, 1, 0, 0, 0, 0, 0, 1, 0x3c})),
        std::runtime_error);
}

TEST_CASE(
    "The encoded frame size limit includes escaping and delimiters", "[serializer][framing]") {
    SECTION("Exactly at the limit") {
        REQUIRE(
            CommandSerializer::packageDataForBt(Buffer(2039, 0), DATA_TYPE::DATA_MDR, 0).size() ==
            2048);
    }
    SECTION("One byte over the limit") {
        REQUIRE_THROWS_AS(
            CommandSerializer::packageDataForBt(Buffer(2040, 0), DATA_TYPE::DATA_MDR, 0),
            std::runtime_error);
    }
    SECTION("Escape expansion exceeds the limit") {
        REQUIRE_THROWS_AS(
            CommandSerializer::packageDataForBt(Buffer(1020, 0x3c), DATA_TYPE::DATA_MDR, 0),
            std::runtime_error);
    }
}
...
TEST_CASE("Legacy NC and ambient command layouts remain unchanged", "[serializer][v1]") {
    ...
    REQUIRE(
        CommandSerializer::serializeNcAndAsmSetting(NC_ASM_EFFECT::ADJUSTMENT_COMPLETION,
...
TEST_CASE("Legacy VPT and positioning serialization remains unchanged", "[serializer][v1]") {
    REQUIRE(
        CommandSerializer::serializeVPTSetting(VPT_INQUIRED_TYPE::VPT, 3) == bytes({0x48, 1, 3}));
    REQUIRE(CommandSerializer::serializeVPTSetting(VPT_INQUIRED_TYPE::SOUND_POSITION, 0x12) ==
            bytes({0x48, 2, 0x12}));
}
